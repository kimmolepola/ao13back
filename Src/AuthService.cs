...
class AuthService
{
        ...
        EmailOptions? emailOptions)
    {
        ...
        app.MapPost("/api/v1/auth/signup", (UserDb db, Signup data) =>
        {
            ...
            var link = clientOptions?.Address + "/confirm-email?token=" + token + "&email=" + data.Email;
            ...
            string message =
                ...
                + link
                + "</body></html>";
            MailMessage mail = new(emailOptions?.FromEmail, data.Email, subject, message) { IsBodyHtml = true };
        ...
        app.MapPost("/api/v1/auth/requestResetPassword", (UserDb db, RequestResetPassword data) =>
        {
            ...
            var link = clientOptions?.Address + "/reset-password?token=" + token + "&email=" + user.Email;
            ...
            string message =
                ...
                + link
                + "</body></html>";
            MailMessage mail = new(emailOptions?.FromEmail, user.Email, subject, message) { IsBodyHtml = true };
            smtpClient.Send(mail);
